<template>
	<div>
		...
		<van-field style="margin-top: 10px;" readonly clickable label="课程" :value="course" placeholder="选择课程" @click="showCoursePicker = true"></van-field>
		<van-field readonly clickable label="截止时间" :value="endTime" placeholder="选择截止时间" @click="showTimePicker = true"></van-field>
		<van-cell-group>
		...
		<van-popup v-model="showCoursePicker" position="bottom">
			<van-picker show-toolbar title="课程" :columns="courseArray" @cancel="showCoursePicker = false" @confirm="onCourseConfirm"></van-picker>
		</van-popup>
		<van-popup v-model="showTimePicker" position="bottom">
			<van-datetime-picker title="截止时间" type="datetime" v-model="currentDate" @cancel="showTimePicker = false" @confirm="onDateConfirm"></van-datetime-picker>
		</van-popup>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			onClickCommit() {
				let that = this
				that.isLoading = true
				axios.post('http://192.168.1.127:8080/uploadHomework', {
					openId: that.$store.state.openId,
					id: that.taskId,
